...
async fn main() -> Result<(), reqwest::Error> {
    ...
    let get_owned_games_request: String = base_owned_games_request + key_add + key + steam_id_add + steam_id;
    ...
    let app_id: &str = "15620"; // TODO HARDCODED FOR NOW

    let base_get_player_achievements_request: String = "https://api.steampowered.com/ISteamUserStats/GetPlayerAchievements/v1/?".to_owned();
    let get_player_achievements_request: String = base_get_player_achievements_request + key_add + key + steam_id_add + steam_id + app_id_add + app_id;
    ...
    // Get details of the first achievement
    let base_get_schema_for_game_request: String = "https://api.steampowered.com/ISteamUserStats/GetSchemaForGame/v2/?key=".to_owned();
    let get_schema_for_game_request: String = base_get_schema_for_game_request + key + app_id_add + app_id;
    ...
    let filter_to_unachieved: Vec<&Achievement> = player_achievements.playerstats.achievements
        .iter()
    ...
    if random_achievement.is_none() {
        ...
        let selected_achievement_desc: Vec<&GameAchievement> = achievements.game.available_game_stats.achievements.iter()
        .filter(|a| a.name==random_achievement.unwrap().apiname)
        .collect();
        println!("{:#?}", selected_achievement_desc.get(0))
    }
    Ok(())
}